...
#if (LLVM_PACKAGE_VERSION >= 900)
#else
ast_matchers::internal::VariadicDynCastAllOfMatcher<Stmt, UnresolvedMemberExpr> unresolvedMemberExpr;
...
AST_POLYMORPHIC_MATCHER_P(hasAnyArgumentFallback, AST_POLYMORPHIC_SUPPORTED_TYPES(CXXUnresolvedConstructExpr), ast_matchers::internal::Matcher<Expr>, InnerMatcher){
	//for (const Expr *Arg : Node.arguments()) {
	for (unsigned I = 0; I < Node.arg_size(); ++I) {
		ast_matchers::internal::BoundNodesTreeBuilder Result(*Builder);
		if (InnerMatcher.matches(*Node.getArg(I), Finder, &Result)) {
...
ast_matchers::internal::VariadicDynCastAllOfMatcher<Stmt, CXXDependentScopeMemberExpr> cxxDependentScopeMemberExpr;
#endif
...
const Stmt *ExprMutationAnalyzer::findDirectMutation(const Expr *Exp) {
  ...
  const auto AsAssignmentLhs =
      binaryOperator(matchers::isAssignmentOperator(), hasLHS(equalsNode(Exp)));
  ...
  const auto AsNonConstRefArg = anyOf(
      ...
      cxxUnresolvedConstructExpr(hasAnyArgumentFallback(equalsNode(Exp))));
