...
// CHECK-MESSAGES-OLD: :[[@LINE-1]]:5: warning: Barrier inside if/else may not be reachable due to reference to ID-dependent variable 'ie_i' in condition at {{(\/)?([^\/\0]+(\/)?)+}}:[[@LINE-2]]:7 [opencl-possibly-unreachable-barrier]
  ...
  if (ie_i < 512) {	
// CHECK-MESSAGES-OLD: :[[@LINE-1]]:5: warning: Barrier inside if/else may not be reachable due to reference to ID-dependent variable 'ie_i' in condition at {{(\/)?([^\/\0]+(\/)?)+}}:[[@LINE-1]]:7 [opencl-possibly-unreachable-barrier]
  } else if (ie_i < 1024) {
    barrier(CLK_LOCAL_MEM_FENCE);
// XCHECK-MESSAGES-OLD: :[[@LINE-1]]:5: warning: Barrier inside if/else may not be reachable due to reference to ID-dependent variable 'ie_i' in condition at {{(\/)?([^\/\0]+(\/)?)+}}:[[@LINE-2]]:7 [opencl-possibly-unreachable-barrier]
  }
  if (ie_i < 1024) {
